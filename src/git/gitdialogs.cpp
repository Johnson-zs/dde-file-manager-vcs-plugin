...
void GitLogDialog::loadCommitHistory()
{
    m_commitTree->clear();
    
    QStringList args {"log", "--oneline", "--graph", "--decorate", "--all", "--format=%h|%s|%an|%ad|%H", "--date=short", "-100"};
    ...
    if (process.waitForFinished(5000)) {
        ...
        for (const QString &line : lines) {
            ...
            if (parts.size() >= 5) {
                ...
                item->setText(0, "●");  // Simple graph placeholder
                item->setText(1, parts[1]);  // Subject
                item->setText(2, parts[2]);  // Author
                item->setText(3, parts[3]);  // Date
                item->setText(4, parts[0]);  // Short hash
                item->setData(0, Qt::UserRole, parts[4]);  // Full hash
            }
        }
    }
}
