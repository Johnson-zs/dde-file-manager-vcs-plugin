...
void GitPullDialog::setupUI()
{
    ...
    m_pullButton->setStyleSheet("QPushButton { font-weight: bold; padding: 8px 16px; }");
...
void GitPullDialog::executeDryRun()
{
    ...
    QMessageBox::information(this, tr("Dry Run"), 
                            tr("Dry run functionality will be implemented in the next phase."));
}
...
{
    bool canPull = !m_remoteCombo->currentText().isEmpty() && 
                   !m_remoteBranchCombo->currentText().isEmpty() &&
