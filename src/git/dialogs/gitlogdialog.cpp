...
void GitLogDialog::setupCommitDetails()
{
    ...
    m_commitDetails->setMaximumHeight(300);
...
void GitLogDialog::loadCommitHistory(bool append)
{
    ...
    if (!currentBranch.isEmpty() && currentBranch != "HEAD" && currentBranch != tr("All Branches")) {
    ...
    qInfo() << QString("INFO: [GitLogDialog] Loaded %1 commits (total offset: %2) for branch: %3")
                       .arg(loadedCount)
                       .arg(m_currentOffset)
                       .arg(m_branchSelector->getCurrentSelection());
...
void GitLogDialog::updateCommitSummaryStats(const QStringList &statLines)
{
    ...
    if (!currentCommitHash.isEmpty()) {
        ...
        // 组合HTML内容
        QString htmlContent = summaryStats + "<hr style='border: 1px solid #ccc; margin: 10px 0;'>" + "<pre style='font-family: Consolas, monospace; font-size: 9pt; margin: 0;'>" + currentDetails.toHtmlEscaped() + "</pre>";
