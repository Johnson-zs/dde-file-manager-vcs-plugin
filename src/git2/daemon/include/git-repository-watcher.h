 ...
 * 
 * 迁移自GitFileSystemWatcher，集成到daemon服务中
 * 核心功能：
 ...
 */
class GitRepositoryWatcher : public QObject
{
    Q_OBJECT

public:
    explicit GitRepositoryWatcher(QObject *parent = nullptr);
     ...
     * @return 状态信息映射
     */
    QHash<QString, QVariant> getWatcherStats() const;
...
private:
    ...
    // 性能统计
    mutable int m_watchEvents;                   ///< 监控事件计数
    mutable int m_updateEvents;                  ///< 更新事件计数
